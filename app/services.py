...
async def create_csv_file(file, session: AsyncSession):
    ...
    with open_csvfile(filename, 'w', newline='') as csvfile:
...
async def calculate_TF_IDF_measure(
    ...
    with open_csvfile(filename, newline='') as csvfile:
        reader = csv.DictReader(csvfile, quoting=csv.QUOTE_NONNUMERIC)
        for row in reader:
            response_result_dict = {word: {'TF': tf} for word, tf in row.items()}
            break

    files_from_db = await get_file_from_db(session)

    total_number_documents = 0

    with open_csvfiles(files_from_db) as csvfile:
        for f in csvfile:
    ...
    return sorted(response_result_dict.items(), key=lambda x: x[1]['IDF'], reverse=True)[:50], files_from_db
