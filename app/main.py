...
async def upload_file(
    ...
    total_number_words, word_list, prepare_string_words = prepare_data(_bytes)
    file_data_from_request = dict(Counter(word_list))
    result = TF_IDF_func(total_number_words, file_data_from_request, prepare_string_words, filename, session)
    return templates.TemplateResponse(
            request=request, name='table.html', context={'result': result, 'length': len(result)}
    )


def prepare_data(_bytes):
...
def TF_IDF_func(total_number_words, file_data_from_request, prepare_string_words, filename, session):
    ...
    with open(FILE_PATH.format(filename=filename), 'w') as f:
        ...
        db.add_file_to_db(session=session, files_data=files_data)

    all_files_from_db = db.get_file_from_db(session)
    ...
    for word, data in response_result_dict.items():
        ...
        data['IDF'] = IDF_Function(total_number_documents, number_documents_with_word)
...
def IDF_Function(total_number_documents, number_documents_with_word):
